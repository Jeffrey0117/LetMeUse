"use strict";(()=>{(function(){let v=document.currentScript??document.querySelector('script[src*="/letmeuse.js"][data-app-id]'),u=v?.getAttribute("data-app-id")??"",W=v?.getAttribute("data-theme")??"light",s=v?.getAttribute("data-accent")??"#2563eb",Z=v?.getAttribute("data-locale")??"en",j=v?.getAttribute("data-mode")??"modal",m=v?.src?new URL(v.src).origin:window.location.origin;u||console.warn("[LetMeUse SDK] Missing data-app-id attribute on script tag.");let D={"title.login":{en:"Sign In",zh:"\u767B\u5165"},"title.register":{en:"Create Account",zh:"\u5EFA\u7ACB\u5E33\u865F"},"label.email":{en:"Email",zh:"\u96FB\u5B50\u4FE1\u7BB1"},"label.password":{en:"Password",zh:"\u5BC6\u78BC"},"label.displayName":{en:"Display Name",zh:"\u986F\u793A\u540D\u7A31"},"btn.login":{en:"Sign In",zh:"\u767B\u5165"},"btn.register":{en:"Create Account",zh:"\u5EFA\u7ACB\u5E33\u865F"},"switch.toRegister":{en:"Don't have an account? Sign up",zh:"\u6C92\u6709\u5E33\u865F\uFF1F\u8A3B\u518A"},"switch.toLogin":{en:"Already have an account? Sign in",zh:"\u5DF2\u6709\u5E33\u865F\uFF1F\u767B\u5165"},"error.generic":{en:"Something went wrong",zh:"\u767C\u751F\u932F\u8AA4"},"msg.loading":{en:"Loading...",zh:"\u8F09\u5165\u4E2D..."},"oauth.or":{en:"or continue with",zh:"\u6216\u4F7F\u7528\u4EE5\u4E0B\u65B9\u5F0F\u767B\u5165"},"oauth.google":{en:"Google",zh:"Google"},"oauth.github":{en:"GitHub",zh:"GitHub"},"link.forgotPassword":{en:"Forgot password?",zh:"\u5FD8\u8A18\u5BC6\u78BC\uFF1F"}};function a(e){return D[e]?.[Z]??D[e]?.en??e}let O=`lmu_${u}_`,R=`${O}access_token`,H=`${O}refresh_token`;function S(){return localStorage.getItem(R)}function F(){return localStorage.getItem(H)}function $(e,t){localStorage.setItem(R,e),localStorage.setItem(H,t)}function M(){localStorage.removeItem(R),localStorage.removeItem(H)}async function C(e,t){let o=await fetch(`${m}${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)}),n=await o.json();if(!o.ok)throw new Error(n.error??a("error.generic"));return n.data??n}async function P(e,t){let o={};t&&(o.Authorization=`Bearer ${t}`);let n=await fetch(`${m}${e}`,{headers:o}),i=await n.json();if(!n.ok)throw new Error(i.error??a("error.generic"));return i.data??i}let k=[];async function ee(){if(u)try{k=(await P(`/api/auth/providers?app_id=${u}`,"")).providers??[]}catch{k=[]}}function q(e){let t=encodeURIComponent(window.location.href);window.location.href=`${m}/api/auth/oauth/${e}?app_id=${u}&redirect=${t}`}let r=null,I=!1,A=[],E=null;function x(){for(let e of A)try{e(r)}catch{}}function te(e){try{let t=JSON.parse(atob(e.split(".")[1]));return t.exp?t.exp*1e3:null}catch{return null}}function T(){E&&clearTimeout(E);let e=S();if(!e)return;let t=te(e);if(!t)return;let o=t-Date.now()-300*1e3;if(o<=0){G();return}E=setTimeout(G,o)}async function G(){let e=F();if(e)try{let t=await C("/api/auth/refresh",{refreshToken:e});$(t.accessToken,t.refreshToken),T()}catch{M(),r=null,x()}}function oe(){let e=window.location.hash;if(!e.includes("lmu_token="))return!1;let t=new URLSearchParams(e.slice(1)),o=t.get("lmu_token"),n=t.get("lmu_refresh");return o&&n?($(o,n),window.history.replaceState(null,"",window.location.pathname+window.location.search),!0):!1}async function Y(){oe(),await ee();let e=S();if(!e){I=!0,x();return}try{r=(await P("/api/auth/me",e)).user,T()}catch{let t=F();if(t)try{let o=await C("/api/auth/refresh",{refreshToken:t});$(o.accessToken,o.refreshToken),r=(await P("/api/auth/me",o.accessToken)).user,T()}catch{M()}else M()}I=!0,x()}let p=W==="dark",_=p?"#1e1e2e":"#ffffff",h=p?"#cdd6f4":"#1e293b",g=p?"#a6adc8":"#64748b",z=p?"#313244":"#f8fafc",f=p?"#45475a":"#e2e8f0",ne=`
    :host {
  ...
  `;function U(e){let t=document.getElementById("lmu-auth-host");t&&t.remove();let o=e,n="",i=!1,l=document.createElement("div");l.id="lmu-auth-host",l.style.cssText="position:fixed;inset:0;z-index:99999;";let d=l.attachShadow({mode:"closed"});function w(){let c=o==="login";d.innerHTML=`
        ...
        <div class="lmu-card">
          ...
          <form id="lmu-auth-form">
            ...
            <div class="lmu-field">
              ...
              <input class="lmu-input" type="password" name="password" required minlength="${c?1:8}" />
            </div>
      ...
      `,d.getElementById("lmu-close-btn")?.addEventListener("click",()=>l.remove()),d.addEventListener("click",B=>{if(B.target===d.firstElementChild?.nextElementSibling)return;let y=d.querySelector(".lmu-card");y&&!y.contains(B.target)&&l.remove()}),d.getElementById("lmu-oauth-google")?.addEventListener("click",()=>q("google")),d.getElementById("lmu-oauth-github")?.addEventListener("click",()=>q("github")),d.getElementById("lmu-forgot-pw")?.addEventListener("click",()=>{l.remove(),window.open(`${m}/login?app=${u}&tab=login`,"_blank")}),d.getElementById("lmu-switch-mode")?.addEventListener("click",()=>{o=c?"register":"login",n="",w()});let V=d.getElementById("lmu-auth-form");V?.addEventListener("submit",async B=>{if(B.preventDefault(),i)return;i=!0,n="",w();let y=new FormData(V),X=y.get("email"),Q=y.get("password");try{if(c){let b=await C("/api/auth/login",{appId:u,email:X,password:Q});$(b.accessToken,b.refreshToken),r=b.user,T(),x(),l.remove()}else{let b=y.get("displayName"),N=await C("/api/auth/register",{appId:u,email:X,password:Q,displayName:b});$(N.accessToken,N.refreshToken),r=N.user,T(),x(),l.remove()}}catch(b){n=b instanceof Error?b.message:a("error.generic"),i=!1,w()}})}w(),document.body.appendChild(l),setTimeout(()=>{d.querySelector("input")?.focus()},50)}let J=`
    :host {
      display: block;
      font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
      line-height: 1.5;
  ...
  `;function ie(e){let t=typeof e=="string"?document.querySelector(e):e;if(!t)return()=>{};let o=document.createElement("div");o.className="lmu-avatar-host";let n=o.attachShadow({mode:"closed"}),i=!1;function l(){if(!r){n.innerHTML=`
        ...
        `:""}
      `,n.getElementById("lmu-av-toggle")?.addEventListener("click",()=>{i=!i,l()}),n.getElementById("lmu-av-profile")?.addEventListener("click",()=>{i=!1,l(),window.open(`${m}/login?app=${u}&tab=profile`,"_blank")}),n.getElementById("lmu-av-logout")?.addEventListener("click",()=>{i=!1,L.logout()})}function d(c){i&&!o.contains(c.target)&&(i=!1,l())}document.addEventListener("click",d),t.appendChild(o),l();let w=L.onAuthChange(()=>{i=!1,l()});return()=>{w(),document.removeEventListener("click",d),o.remove()}}let L={get ready(){return I},get user(){return r},login(){if(j==="redirect"){window.location.href=`${m}/login?app=${u}&redirect=${encodeURIComponent(window.location.href)}`;return}U("login")},register(){if(j==="redirect"){window.location.href=`${m}/login?app=${u}&redirect=${encodeURIComponent(window.location.href)}&tab=register`;return}U("register")},async logout(){let e=S();if(e)try{await fetch(`${m}/api/auth/logout`,{method:"POST",headers:{Authorization:`Bearer ${e}`}})}catch{}M(),r=null,E&&clearTimeout(E),x()},getToken(){return S()},onAuthChange(e){if(A.push(e),I)try{e(r)}catch{}return()=>{let t=A.indexOf(e);t!==-1&&A.splice(t,1)}},openAdmin(){window.open(`${m}/admin`,"_blank")},renderProfileCard(e){return re(e)},renderAvatar(e){return ie(e)}};window.letmeuse=L,document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>Y()):Y()})();})();
